    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                ...
                history.append({"iter": iteration, "loss": ema_loss_for_log})
...
if __name__ == "__main__":
    ...
    history = training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations,
    ...
    with open(output_file, "w") as f:
        for dict in history:
            # 将每条记录写入文件
            f.write(f"iter: {dict['iter']}, loss: {dict['loss']:.7f}\n")
